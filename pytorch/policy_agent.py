...
def REINFORCE(training_pairs, model, optimizer, num_episodes):
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            reward, new_state, done = env.interact(state_idx, action_chosen)  # new_state: 전체 elimited_kb에서 무작위로 찾은 valid new state
        ...
        if len(state_batch_negative) != 0:
            ...
            # print("2up")
            update(model, np.reshape(state_batch_negative, (-1, state_dim)), -0.05, action_batch_negative, optimizer)  # -0.05를 loss에 곱해줌
        ...
        if done == 1:
            ...
            # print("3up")
            update(model, np.reshape(state_batch, (-1, state_dim)), total_reward, action_batch, optimizer)
        else:
            global_reward = -0.05
...
def retrain():
    ...
    optimizer = torch.load('torchmodels/policy_supervised_' + relation + "_opt.pth")
    ...
    torch.save(optimizer, "torchmodels/policy_retrained_" + relation + "_opt.pth")
...
def test():
    ...
    optimizer = torch.load('torchmodels/policy_retrained_' + relation + "_opt.pth")
    ...
    for episode in range(test_num):
        ...
        if done:
            if len(path_set) != 0:
                path_found_embedding = [env.path_embedding(path.split(" -> "), embedding_dim) for path in path_set]
                curr_path_embedding = env.path_embedding(env.path_relations, embedding_dim)
                path_found_embedding = np.reshape(path_found_embedding, (-1, embedding_dim))
                cos_sim = cosine_similarity(path_found_embedding, curr_path_embedding)
                diverse_reward = -np.mean(cos_sim)
                print("diverse_reward", diverse_reward)
                ...
                for t, transition in enumerate(transitions):
                    if transition.reward == 0:
                        state_batch.append(transition.state)
                        action_batch.append(transition.action)
                update(model, np.reshape(state_batch, (-1, state_dim)), 0.1*diverse_reward, action_batch, optimizer)
