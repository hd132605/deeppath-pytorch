...
class Env:
	...
	#
	#
	# 	# entity2id.txt와 relation2id.txt를 Env __init__에서 읽던 걸 밖에서 읽는 방식으로 변경.
	...
	def interact(self, state, action):
		...
		choices = []
		for line in self.eliminated_kb:  # 모든 eliminated_kb를 스캔해서...
			triple = line.rsplit()
			e1_idx = self.entity2id[triple[0]]
			
			if curr_pos == e1_idx and triple[2] == chosen_relation and triple[1] in self.entity2id:  # choices에 맞는 선택만 골라 넣은 다음...
		...
		if len(choices) == 0:
			...
			self.path.append(path[2] + ' -> ' + path[1])  # 맞는 triple을 골라 self.path에 append.
