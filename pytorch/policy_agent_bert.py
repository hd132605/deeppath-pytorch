...
max_episode_num = 10  # original code : 300
max_test_num = 10  # original code : 500
...
def update(model:torch.nn.Module,
    ...
    state = torch.tensor(state, dtype=torch.float).to(device)
    action = torch.tensor(action).to(device)  # creating tensor from a list of ndarrays is slow?
...
def REINFORCE(training_pairs, model, optimizer, num_episodes):
    ...
    for i_episode in range(num_episodes):
        ...
        env = Env(entity2vec, relation2vec, kb_all, eliminated_kb, entity2id, relation2id, relations, train[i_episode])

        sample = train[i_episode].split()
        state_idx = [entity2id[sample[0]], entity2id[sample[1]], 0]
        ...
        if len(state_batch_negative) != 0:
            ...
            # print("2up")
            update(model, np.reshape(state_batch_negative, (-1, applied_state_dim)), -0.05, action_batch_negative, optimizer)
        ...
        if done == 1:
            ...
            # print("3up")
            update(model, np.reshape(state_batch, (-1, applied_state_dim)), total_reward, action_batch, optimizer)
        else:
            global_reward = -0.05
            ...
            try:
                ...
                for item in good_episodes:
                    ...
                    # print("5up")
                    update(model, np.squeeze(teacher_state_batch), 1, teacher_action_batch, optimizer)
    ...
    relation_path_stats = list(collections.Counter(path_relation_found).items())
...
def retrain():
    ...
    checkpoint = torch.load("torchmodels/" + model_save_dir + "/policy_supervised_" + relation + ".pth")
    model = PolicyNeuralNetTorchForBert(applied_state_dim)
    ...
    print("sl_policy for " + mode + " restored")
    ...
    }, "torchmodels/" + model_save_dir + "/policy_retrained_" + relation + ".pth")
    print("Retrained model for " + mode + " saved")
...
def test():
    ...
    checkpoint = torch.load("torchmodels/" + model_save_dir + "/policy_retrained_" + relation + ".pth")
    model = PolicyNeuralNetTorchForBert(applied_state_dim)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.to(device)
    ...
    for episode in range(test_num):
        ...
        env = Env(entity2vec, relation2vec, kb_all, eliminated_kb, entity2id, relation2id, relations, test_data[episode])
        ...
        if done:
            if len(path_set) != 0:
                # change embedding_dimmension value w.r.t. mode
                if mode in ["bert", "kgbert", "kgbert_triple"]:
                    applied_embedding_dim = embedding_dim_bert
                elif mode == "transr_kgbert_concat":
                    applied_embedding_dim = embedding_dim_transr_kgbert_concat
                else:
                    raise ValueError

                path_found_embedding = [env.path_embedding(path.split(" -> "), applied_embedding_dim) for path in path_set]
                curr_path_embedding = env.path_embedding(env.path_relations, applied_embedding_dim)
                path_found_embedding = np.reshape(path_found_embedding, (-1, applied_embedding_dim))
                cos_sim = cosine_similarity(path_found_embedding, curr_path_embedding)
                diverse_reward = -np.mean(cos_sim)
                print("diverse_reward", diverse_reward)
                # total_reward = 0.1*global_reward + 0.8*length_reward + 0.1*diverse_reward
                ...
                for t, transition in enumerate(transitions):
                    if transition.reward == 0:
                        state_batch.append(transition.state)
                        action_batch.append(transition.action)
                update(model, np.reshape(state_batch, (-1, applied_state_dim)), 0.1*diverse_reward, action_batch, optimizer)
    ...
    with open(get_path_to_use_path(mode, relation), "w") as f:
