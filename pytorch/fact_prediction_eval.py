		...
		if len(left) < len(right):
			...
			for entity in left:
				try:
					...
					# print 'left', len(left)
					print(('left', len(left)))
					print(('left', len(left)), file=open(log_file_name, encoding="utf-8", mode="at"))
					# print left
					print(left)
					print(left, file=open(log_file_name, encoding="utf-8", mode="at"))
					# print 'not such entity'
					print('not such entity')
					print('not such entity', file=open(log_file_name, encoding="utf-8", mode="at"))
					return False
			left = left_next
			...
			for entity in right:
				try:
					...
					print(('right', len(right)))
					print(('right', len(right)), file=open(log_file_name, encoding="utf-8", mode="at"))
					# print 'no such entity'
					print('no such entity')
					print('no such entity', file=open(log_file_name, encoding="utf-8", mode="at"))
					return False
			right = right_next
...
def get_features():
	...
	for line in paths:
		...
		if path not in stats:
			...
			continue  # 강화학습 에이전트가 찾은(policy_agent.py test sth) path라도 path frequency(random)가 1이하면 사용하지 않겠다.

		length = len(path.split(' -> '))

		if length <= 10:  # path length(사용한 relation의 수)가 10 이하인 path만 사용하겠다.(너무 길면 컷!)
			pathIndex = []
			pathName = []
	...
	print(('How many paths used: ', len(useful_paths)))
	print(('How many paths used: ', len(useful_paths)), file=open(log_file_name, encoding="utf-8", mode="wt"))
...
print(('How many queries: ', len(test_pairs)))
print(('How many queries: ', len(test_pairs)), file=open(log_file_name, encoding="utf-8", mode="at"))
for idx, sample in enumerate(test_pairs):  # test_pairs: [(e1, e2), (e1, e2), ...]
...
rank_stats_E.sort(key = lambda x:x[0], reverse=True)  # score_E가 높은 순서대로 정렬(e1_vec + rel - e2_vec). 그러면 전부 음수지만 0에 가까울수록 맞게 예측한 것이다.
